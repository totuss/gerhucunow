...
import type { User, LogEntry, Account } from "./types"

export default function FileCheckStatistics() {
  ...
  const handleUpdateSubscription = (username: string, days: number) => {
    setUsers((prevUsers) =>
      prevUsers.map((user) =>
        user.username === username ? { ...user, daysLeft: Math.max(0, user.daysLeft + days) } : user,
      ),
    )
  }
  ...
  if (isAdmin) {
    return <AdminPanel users={users} onUpdateSubscription={handleUpdateSubscription} onLogout={handleLogout} />
  }

  // If subscription expired, show message
